...
torchrun --nproc_per_node=$NUM_PROC --master_port $MASTER_PORT main.py \
  	...
  	--cali_bsz 2 --cali_bsz_accumulate_step 4 --epoch $EPOCH --flat_lr 5e-3 \
        ...
        --output_dir ./outputs/lr_ddp_${NUM_DDP_SIZE}_flat${QAT_FILE}_bsz2_accum4_epoch${EPOCH}_nsamples${NSAMPLES} \
        --quantized_save --blockwise_save \
        --cali_dataset redpajama \
        --nsamples $NSAMPLES

bash run_ppl_eval.sh ./outputs/lr_ddp_${NUM_DDP_SIZE}_flat${QAT_FILE}_bsz2_accum4_epoch${EPOCH}_nsamples${NSAMPLES}/llama-3.1-8b-instruct/w4a4/exp/